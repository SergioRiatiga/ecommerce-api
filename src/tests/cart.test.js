...
test('POST -> "URL_BASE", should return status code 201, res.body.quantity === bodyCart.quantity', async () => {
  const bodyCart = {
    quantity:1,
...
test('PUT -> "URL_BASE/:id",should return status code 200, res.body.quantity === bodyUpdate.quantity', async () => {
  const bodyUpdate = {
    quantity: 2
  }
  const res = await request(app)
