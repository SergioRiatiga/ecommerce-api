...
test('POST -> "URL_BASE", should status code 201, res.body.url to be defined, res.body.file to be defined', async () => {
...
test('GET -> "URL_BASE", should status code 200, res.body.length === 1', async () => {
  const res = await request(app)
    .get(URL_BASE)
    .set("Authorization", `Bearer ${TOKEN}`)
